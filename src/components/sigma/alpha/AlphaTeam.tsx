...
const TEAM: { name: string; realName: string; role: string; glyph: string; accent: string; skills: string[] }[] = [
  ...
  { name: "NULL CIPHER", realName: "Lin Htet", role: "Security", glyph: "⚿", accent: "#B85C2E", skills: ["Audits", "Pen Test", "Crypto"] },
  { name: "GHOST PRINTER", realName: "Zaw Ye", role: "Hardware", glyph: "⚙", accent: "#FFEB3B", skills: ["PCB", "Oscilloscope", "Rework"] },
];

/**
 * TeamMemberCard — wraps each SciFiCard in a 3D-tilt layer (max 4deg,
 * perspective 800px). useTilt3D self-disables for touch devices +
 * prefers-reduced-motion, so this is desktop-pointer-only by design.
 */
function TeamMemberCard({ m, i }: { m: typeof TEAM[number]; i: number }) {
  const tiltRef = useTilt3D<HTMLDivElement>(4);
  return (
    <div ref={tiltRef} className="alpha-tilt-card">
      <SciFiCard accent={m.accent} label={`OP·${String(i + 1).padStart(2, "0")}`} id={m.role} className="alpha-card-hover" style={{ "--sigma-hover-accent": m.accent } as React.CSSProperties}>
        <div className="p-3">
          ...
          <div className="relative mb-3 overflow-hidden border border-border/50">
            ...
            <div className="flex items-center justify-center p-3 sm:p-4">
              <span
                className="alpha-img-scale sigma-spin-slow font-sans text-3xl font-black sm:text-5xl"
          ...
          {/* Real identity — italic serif subtitle flanked by accent rules.
              The trailing rule uses .alpha-name-underline so it draws L→R on hover. */}
          <div className="mt-0.5 flex items-center gap-1.5">
            ...
            <span
              className="alpha-name-underline h-px flex-1"
              style={{ background: m.accent, opacity: 0.5 }}
              aria-hidden
            />
          </div>
...
export function AlphaTeam() {
  return (
    <section id="team" aria-labelledby="team-title" data-section="team" className="relative border-t border-border px-3 py-12 sm:px-6 sm:py-20">
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1600px]">
        ...
        <div className="flex flex-col gap-3 border-b border-border pb-4 sm:flex-row sm:items-end sm:justify-between sm:gap-4">
          <div>
            ...
            <h2 id="team-title" className="mt-2 font-sans text-3xl font-black uppercase leading-tight tracking-tight sm:text-5xl md:text-6xl">
        ...
        {/* Team grid — SciFiCard dossier style, each wrapped in a 3D-tilt layer */}
